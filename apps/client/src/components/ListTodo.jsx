...
async function getTodos() {
    const res = await fetch("project/itemStatus");
        ...
        <tbody>
          {todos.map((todo) => (
            <tr key={todo.todo_id} style={todo.completed === true ? {textAlign:"center", backgroundColor:"green", color:"white", textDecoration: 'line-through'}: {textAlign:'center'}}>
              ...
              <td><a href="/main" selectedProject={todo.project_id} todoid={todo.todo_id} >{todo.project_id}</a></td>
              <td><input type="checkbox" value={todo.completed} name="completed" onChange={(e)=> handleCheck(e, todo.todo_id)} checked={todo.completed === false? false: true}/></td>
              <td>
                <EditTodo todo={todo} />
              </td>
              <td>
